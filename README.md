...
| Событие | Рейтинг |
|---|---|
| Доставка в срок (рейс закончился не позже дедлайна) | `+3` |
...
Числа выше взяты из постановки задачи. Симуляция показывает, что с ними партия
почти не выживает: при жадной игре без простоев до седьмых суток дотягивает
примерно 1 прогон из 10 — кредитов накапливается кратно больше нужного, но
рейтинг съедают протухшие заказы, которые три ровера физически не успевают
разобрать.

Проверить и подобрать свои числа можно, не трогая код:

```bash
npm run simulate --workspace server -- --runs 20        # текущий баланс
BONUS_ON_TIME=10 npm run simulate --workspace server    # 10 из 10 доживают
PENALTY_EXPIRED=5 npm run simulate --workspace server   #  9 из 10 доживают
```

Те же переменные (`BONUS_ON_TIME`, `PENALTY_EXPIRED`, `PENALTY_DECLINED`,
`ORDER_EVERY_HOURS`, `CHARGE_PER_HOUR`) читает и сервер, так что запустить игру с
другим балансом можно тем же способом.
