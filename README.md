...
| Событие | Рейтинг |
|---|---|
| Доставка в срок (рейс закончился не позже дедлайна) | `+12` |
...
Итог 80 прогонов на текущих значениях:

| Показатель | Значение |
|---|---|
| дожили до седьмых суток | 74% |
| победили (7 суток и 28 000 ₡) | **70%**, то есть 7 партий из 10 |
| кредитов у дошедших до финала | p10 28 648, медиана 33 921, максимум 44 788 |
| доставок за партию | ≈ 21 при ≈ 20 просроченных |
| рейтинг на финише | медиана 54 |

Победа реальна, но не гарантирована, и это результат двух настроек сразу.
Бонус за доставку в срок отвечает за выживаемость: при `+10` до финала доходили
64% партий, при `+12` — 74%. Цель по кредитам срезает лучшую часть оставшихся:
25 000 не отсекает почти никого, 30 000 роняет долю до 63%, 32 000 — до 55%.
28 000 оставляет 70% и при этом отбирает партии, где риск съел слишком много рейсов.

Проверить и подобрать свои числа можно, не трогая код:

```bash
npm run simulate --workspace server -- --runs 40
npm run simulate --workspace server -- --runs 80 --thresholds 25000,28000,30000
BONUS_ON_TIME=10 npm run simulate --workspace server -- --runs 40
```
